...
async function accountCreationExample() {
  const randomPrefix = `edgio-api-example-${crypto
    .randomBytes(10)
    .toString("hex")}-`

  if (
    process.env.JWT_ACCESS_KEY?.length &&
    !process.env.ORGANIZATION_ID?.length
  ) {
    throw new Error(
      "JWT_ACCESS_TOKEN are not yet capable to create organizationm, please provide an existing ORGANIZATION_ID"
    )
  }
  ...
  try {
    ...
    console.log(
      "\n> Here is your property. Notice that it already has a production environment linked:",
      createdProperty
    )
    ...
    if (createdOrganization || createdProperty) {
      ...
      if (createdOrganization) {
        console.log(
          `Deleting organization ${createdOrganization.name}, which will cascade delete the rest`
        )
      ...
      if (createdProperty) {
        console.log(
          `Deleting property "${createdProperty.slug}", which will cascade delete the rest`
        )
